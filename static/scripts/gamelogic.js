...
mywords = []
class question {
  constructor(answer,english,hindi,s) {
    this.hindi = hindi;
...
sendandget = function(g,bubble)
{
				...
				xhr2.onreadystatechange = sendAnswer;
			
				xhr2.open("GET","http://localhost:5000/sem_sim/"+answer,false);
		...
		var xhr3 = new XMLHttpRequest();
			function checkAnswer()
			{
				if(xhr3.readyState==4&&xhr3.status==200)
				{
					...
					english_global = resp.result.english_word;
				}
			}


				//send API request and update the global variables
				xhr3.onreadystatechange = checkAnswer;
				xhr3.open("GET","http://localhost:5000/get_a_word",false);
				xhr3.send();
		bubble.get.remove();
...
Game.prototype.addBubble = function(s) {
	var randomX = random(this.platform.screen.width() - 70);
...
var OverState = function(g) {
	...
	if(g instanceof Game) { 
		...
		this.enter = function() {
			...
			var sum = 0;
			for(var i = 0;i<6;i++)
			{sum += mywords[i].score*100}
			var result = $('<h4>Hi, '+g.username+'! Here are your scores.</h4><p><i>Well Done!</i></p><table width = 100%><tr><td>Sl. No.</td><td>	Question</td><td>Your Answer</td><td>Actual Answer</td><td>Score</td></tr><tr><td>1</td><td>'+mywords[0].hindi+'</td><td>'+mywords[0].answer+'</td><td>'+mywords[0].english+'</td><td>'+parseFloat(mywords[0].score*100).toFixed(2)+'</td></tr><tr><td>2</td><td>'+mywords[1].hindi+'</td><td>'+mywords[1].answer+'</td><td>'+mywords[1].english+'</td><td>'+parseFloat(mywords[1].score*100).toFixed(2)+'</td></tr><tr><td>3</td><td>'+mywords[2].hindi+'</td><td>'+mywords[2].answer+'</td><td>'+mywords[2].english+'</td><td>'+parseFloat(mywords[2].score*100).toFixed(2)+'</td></tr><tr><td>4</td><td>'+mywords[3].hindi+'</td><td>'+mywords[3].answer+'</td><td>'+mywords[3].english+'</td><td>'+parseFloat(mywords[3].score*100).toFixed(2)+'</td></tr><tr><td>5</td><td>'+mywords[4].hindi+'</td><td>'+mywords[4].answer+'</td><td>'+mywords[4].english+'</td><td>'+parseFloat(mywords[4].score*100).toFixed(2)+'</td></tr><tr><td>6</td><td>'+mywords[5].hindi+'</td><td>'+mywords[5].answer+'</td><td>'+mywords[5].english+'</td><td>'+parseFloat(mywords[5].score*100).toFixed(2)+'</td></tr><tr><td></td></td><td></td><td></td><td><b>TOTAL : </b></td><td><b>'+parseFloat(sum).toFixed(2)+'</b></td></table>');
			g.platform.stopAnim();
			this.hideMenu();
